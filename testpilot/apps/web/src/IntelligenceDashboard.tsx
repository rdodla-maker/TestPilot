...
function sortItems(a: string, b: string, mode: SortMode, confidenceA?: number, confidenceB?: number): number {
  return mode === 'confidence' ? (confidenceB || 0) - (confidenceA || 0) : a.localeCompare(b);
}

function filterItems<T extends { confidence?: number }>(items: T[], search: string, sort: SortMode, label: (item: T) => string): T[] {
  return [...items]
    .filter((item) => label(item).toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => sortItems(label(a), label(b), sort, a.confidence, b.confidence));
}
...
function EvidenceButtons({ ids, evidence, onEvidence }: { ids: string[]; evidence: IntelligenceEvidence[]; onEvidence: (item: IntelligenceEvidence) => void }) {
  return <div className="evidence-buttons">{ids.map((id) => {
    const item = evidence.find((candidate) => candidate.id === id);
    return item ? <button type="button" key={id} onClick={() => onEvidence(item)}>{id}</button> : <span className="missing-evidence" key={id}>{id} unavailable</span>;
  })}</div>;
}
...
function CollectionSection({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <section className="dashboard-section"><div className="section-heading"><div><p className="eyebrow">Explorer</p><h2>{title}</h2><p>{description}</p></div></div>{children}</section>;
}

function Overview({ payload, profile, snapshot, counts, evidence, onEvidence }: { payload: IntelligenceSnapshot['payload']; profile: ApplicationProfile; snapshot: IntelligenceSnapshot; counts: Record<string, number>; evidence: IntelligenceEvidence[]; onEvidence: (item: IntelligenceEvidence) => void }) {
  const inferredItems = [payload.application.type, payload.application.purpose, payload.application.businessDomain].filter((item): item is NonNullable<typeof item> => item !== undefined);
  return <section className="dashboard-section overview-section"><div className="hero-grid"><div className="hero-copy"><p className="eyebrow">Current intelligence</p><h2>{payload.application.purpose?.value || 'Purpose not determined'}</h2><p>{profile.targetUrls[0] || 'Target URL not determined'}</p><div className="fact-line"><FactBadge type="observed" /><span>{payload.application.name.value || 'Application name not determined'}</span></div><div className="fact-line"><FactBadge type="inferred" /><span>{payload.application.type?.value || 'Application type not determined'}</span></div></div><div className="confidence-panel"><span>Overall confidence</span><strong>{Math.round(snapshot.confidence * 100)}%</strong><Confidence value={snapshot.confidence} /></div></div><div className="metric-grid">{Object.entries(counts).map(([label, value]) => <Metric key={label} value={value} label={label} />)}<Metric value={`v${snapshot.snapshotVersion}`} label="snapshot" /></div><div className="profile-band"><div><span className="eyebrow">Application profile</span><h3>{profile.name || 'Not determined'}</h3><p>{payload.application.businessDomain?.value || 'Not determined'} · schema {snapshot.schemaVersion}</p></div><div className="profile-facts"><span>Last analyzed <strong>{formatDate(snapshot.createdAt)}</strong></span><span>Execution <strong>{snapshot.sourceExecutionId}</strong></span></div></div><div className="overview-columns"><div><h3>What is observed</h3><p className="muted">Directly captured from the application surface.</p><div className="fact-line"><FactBadge type="observed" /><span>{counts.pages} page surface{counts.pages === 1 ? '' : 's'}</span></div><div className="fact-line"><FactBadge type="observed" /><span>{counts.evidence} evidence items</span></div></div><div><h3>What is inferred</h3><p className="muted">Structured conclusions grounded in linked evidence.</p>{inferredItems.map((item) => <button className="inference-link" type="button" key={item.value || 'empty'} onClick={() => onEvidence(evidence.find((entry) => entry.id === item.evidenceIds[0]) || evidence[0])}>{item.value} <Confidence value={item.confidence} /></button>)}</div></div></section>;
}

function PageCard({ item, evidence, onEvidence }: { item: IntelligencePage; evidence: IntelligenceEvidence[]; onEvidence: (item: IntelligenceEvidence) => void }) {
  return <ItemCard type="observed"><div className="card-top"><span className="entity-id">{item.id}</span><FactBadge type="observed" /></div><h3>{item.title || 'Untitled page'}</h3><a className="safe-url" href={item.url} target="_blank" rel="noreferrer">{item.url}</a>{item.purpose && <p className="inferred-text"><FactBadge type="inferred" />{item.purpose.value}</p>}<EvidenceList ids={item.evidenceIds} evidence={evidence} /><EvidenceButtons ids={item.evidenceIds} evidence={evidence} onEvidence={onEvidence} /></ItemCard>;
}

function FeatureCard({ item, evidence, onEvidence }: { item: IntelligenceFeature; evidence: IntelligenceEvidence[]; onEvidence: (item: IntelligenceEvidence) => void }) {
  return <ItemCard><div className="card-top"><span className="entity-id">{item.id}</span><FactBadge type={item.type} /></div><h3>{item.name}</h3><Confidence value={item.confidence} />{item.description && <p>{item.description}</p>}<p className="muted">Related pages: {item.relatedPageIds.join(', ') || 'None'}</p><EvidenceList ids={item.evidenceIds} evidence={evidence} /><EvidenceButtons ids={item.evidenceIds} evidence={evidence} onEvidence={onEvidence} /></ItemCard>;
}

function RoleCard({ item, evidence, onEvidence }: { item: IntelligenceRole; evidence: IntelligenceEvidence[]; onEvidence: (item: IntelligenceEvidence) => void }) {
  return <ItemCard><div className="card-top"><span className="entity-id">{item.id}</span><FactBadge type={item.type} /></div><h3>{item.role}</h3><Confidence value={item.confidence} /><EvidenceButtons ids={item.evidenceIds} evidence={evidence} onEvidence={onEvidence} /></ItemCard>;
}

function WorkflowCard({ item, evidence, onEvidence }: { item: IntelligenceWorkflow; evidence: IntelligenceEvidence[]; onEvidence: (item: IntelligenceEvidence) => void }) {
...
function RiskCard({ item, evidence, onEvidence }: { item: IntelligenceRisk; evidence: IntelligenceEvidence[]; onEvidence: (item: IntelligenceEvidence) => void }) {
  return <ItemCard><div className="card-top"><span className="entity-id">{item.id}</span><FactBadge type={item.type} /></div><h3>Potential QA risk · {item.area}</h3><Confidence value={item.confidence} />{item.reason && <p>{item.reason}</p>}<EvidenceButtons ids={item.evidenceIds} evidence={evidence} onEvidence={onEvidence} /></ItemCard>;
}

function EvidenceSection({ evidence, selected, onSelect }: { evidence: IntelligenceEvidence[]; selected: IntelligenceEvidence | null; onSelect: (item: IntelligenceEvidence) => void }) {
  return <CollectionSection title="Evidence inspector" description="Structured observations behind TestPilot's conclusions."><div className="evidence-table">{evidence.length ? evidence.map((item) => <button type="button" className={selected?.id === item.id ? 'evidence-table-row selected' : 'evidence-table-row'} key={item.id} onClick={() => onSelect(item)}><strong>{item.id}</strong><span>{item.source || 'Source not specified'}</span><p>{item.text}</p></button>) : <EmptyState label="Evidence" />}</div></CollectionSection>;
}

function ComparisonResult({ comparison }: { comparison: SnapshotComparison }) {
  const groups: Array<[string, string[], string[]]> = [['Pages', comparison.addedPages, comparison.removedPages], ['Features', comparison.addedFeatures, comparison.removedFeatures], ['Roles', comparison.addedRoles, comparison.removedRoles], ['Workflows', comparison.addedWorkflows, comparison.removedWorkflows], ['Risks', comparison.addedRisks, comparison.removedRisks]];
  return <div className="comparison-result"><div className="change-summary"><strong>Snapshot comparison</strong><span>{groups.reduce((total, [, added, removed]) => total + added.length + removed.length, 0)} structural changes</span></div>{groups.map(([label, added, removed]) => <div className="change-group" key={label}><strong>{label}</strong>{added.map((item) => <span className="change added" key={`add-${item}`}>+ {item}</span>)}{removed.map((item) => <span className="change removed" key={`remove-${item}`}>− {item}</span>)}{added.length === 0 && removed.length === 0 && <span className="change none">No changes</span>}</div>)}{comparison.changedApplication && <span className="change changed">Application identity changed</span>}</div>;
}
...
export default function IntelligenceDashboard({ projectId, refreshToken }: Props) {
  ...
  const evidence = safeArray(payload?.evidence);
  const pages = safeArray(payload?.pages);
  const features = safeArray(payload?.features);
  const roles = safeArray(payload?.roles);
  const workflows = safeArray(payload?.workflows);
  const risks = safeArray(payload?.risks);

  const filteredPages = useMemo(() => {
    return [...pages].filter((item) => `${item.title || ''} ${item.url}`.toLowerCase().includes(search.toLowerCase())).sort((a, b) => sortItems(a.title || a.url, b.title || b.url, sort));
  }, [pages, search, sort]);
  const filteredFeatures = useMemo(() => filterItems(features, search, sort, (item) => item.name), [features, search, sort]);
  const filteredRoles = useMemo(() => filterItems(roles, search, sort, (item) => item.role), [roles, search, sort]);
  const filteredWorkflows = useMemo(() => filterItems(workflows, search, sort, (item) => item.name), [workflows, search, sort]);
  const filteredRisks = useMemo(() => filterItems(risks, search, sort, (item) => item.area), [risks, search, sort]);
  ...
  const counts = { pages: pages.length, features: features.length, roles: roles.length, workflows: workflows.length, risks: risks.length, evidence: evidence.length };
  ...
  return <div className="dashboard-shell">
    <aside className="dashboard-sidebar">
      ...
      <nav aria-label="Intelligence sections">{sections.map((item, index) => <button className={section === item.id ? 'nav-item active' : 'nav-item'} type="button" key={item.id} onClick={() => setSection(item.id)}><span className="nav-index">{String(index + 1).padStart(2, '0')}</span>{item.label}</button>)}</nav>
    ...
    <main className="dashboard-main">
      ...
      {section === 'overview' && <Overview payload={payload} profile={profile} snapshot={snapshot} counts={counts} evidence={evidence} onEvidence={setSelectedEvidence} />}

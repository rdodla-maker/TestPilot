...
describe('IntelligenceDashboard', () => {
  ...
  it('fails safely when snapshot arrays are malformed', async () => {
    vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL) => {
      const url = String(input);
      if (url.endsWith('/profile')) return Promise.resolve(new Response(JSON.stringify({ success: true, data: { id: 'project-1', name: 'Test App', targetUrls: ['https://example.com'], status: 'active', currentSnapshotId: 'snapshot-2', createdAt: payload.generatedAt, updatedAt: payload.generatedAt } })));
      if (url.endsWith('/intelligence')) return Promise.resolve(new Response(JSON.stringify({ success: true, data: { ...snapshot, payload: { ...payload, pages: null, features: null, roles: null, workflows: null, risks: null, evidence: null } } })));
      if (url.endsWith('/intelligence/snapshots')) return Promise.resolve(new Response(JSON.stringify({ success: true, data: [] })));
      return Promise.resolve(new Response(JSON.stringify({ success: true, data: snapshot })));
    }));

    render(<IntelligenceDashboard projectId="project-1" refreshToken={1} />);
    expect(await screen.findAllByText('Test App')).not.toHaveLength(0);
